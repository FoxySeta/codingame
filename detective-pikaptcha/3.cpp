...
pair<int, int> mobius (int oldY, int oldX, int height, int width);
...
int main() {
    ...
    do {
        int newPikaDir {(pikaDir + dir.size() + side) % dir.size()},
            ...
            auto mob = mobius(newPikaY, newPikaX, height, width);
            newPikaY = mob.first;
            newPikaX = mob.second;
...
pair<int, int> mobius (int oldY, int oldX, int height, int width) {
    if (oldX < 0 || oldX >= width)
        return {oldY, (oldX + width) % width};
    if (oldY < 0 || oldY >= height)
        return {(oldY + height) % height, oldX < width / 2 ? oldX + width / 2 : oldX - width / 2};
    return {oldY, oldX};
}